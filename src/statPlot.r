...
par(mar=c(4,3.7,1,0)+.1, mfrow=c(2,1), xpd=F, cex=3)
plot(as.factor(a0$Medium[which(is.finite(a0$dNdS) & a0$dNdS!=0)]),log10(a0$dNdS[which(is.finite(a0$dNdS) & a0$dNdS!=0)]), pch=3, xlab="Medium", ylab="log10( dN/dS )", col=cOl[1], cex=1)
...
plot(density(a1[[2]]), col=cOl[2], xlab="log10( dN/dS )", main="", lwd=3)
lines(density(a1[[1]]), col=cOl[3], lty=2, lwd=3)
