...
# out: NA
# arg: 0
# date: 20230317, 20230324 (nucleotide ambiguity)

#https://bioinformatics.cvr.ac.uk/calculating-dnds-for-ngs-datasets/
ptIN = "../raw/"; ptOT = "../out/"
ptREF = paste0(ptIN,"ncbi-genomes-2023-03-23/")
rEf = read.csv(paste0(ptIN,"codon.csv"), header=T)
nT = read.csv(paste0(ptIN,"nt_amb.csv"), header=T)
rEf$cDNA = gsub("u","t",rEf$RNA) # DNA2protein translation
nUcleotide = c("a","t","c","g")

##### functions #####
nFlip = function(x="a",sTrand="-"){x0 = data.frame(c("a","t","c","g"),c("t","a","g","c"));return(if(sTrand=="-"){x0[which(x0[,1]==tolower(x)),2]}else{tolower(x)})}
codon2aa = function(cOdon="atg", dIct = rEf){return(dIct$aa[grep(tolower(cOdon),dIct$cDNA)])} ## codon 2 protein

n2p = function(cOdon="atg", n = nUcleotide, aMbiguity = nT){
	if(all(strsplit(cOdon,"")[[1]] %in% n)){return(codon2aa(cOdon))}
	else{
		n0 = strsplit(cOdon,"")[[1]]
		if(any(n0=="z")){return("-")}; n9 = ";"
		n1 = which(n0 %in% nT$code)
		n2 = 1; for(i in 1:length(n1)){n2 = n2*length(strsplit(nT$nucleotide[which(nT$code==n0[n1[i]])],n9)[[1]])}
		n3 = vector(mode="list", length=length(n0)); for(i in 1:length(n3)){if(n0[i] %in% n){n3[[i]] = n0[i]}else{n3[[i]] = strsplit(nT$nucleotide[which(nT$code==n0[i])],n9)[[1]]}}
